...
#[cfg(target_env = "p3")]
use wasip3::{cli::types::ErrorCode, wit_bindgen::FutureRead};
...
impl Stdout {
    ...
    pub async fn flush(&mut self) -> Result<()> {
        let (tx, rx) = wasip3::wit_stream::new();
        let old_stream = std::mem::replace(&mut self.stream, AsyncOutputStream::new(tx));
        drop(old_stream);
        let result = self.completion.as_mut().await.map_err(into_io_error);

        self.completion = Box::pin(wasip3::cli::stdout::write_via_stream(rx).into_future());
...
impl Stderr {
    ...
    pub async fn flush(&mut self) -> Result<()> {
        ...
        self.completion = Box::pin(wasip3::cli::stderr::write_via_stream(rx).into_future());
