...
/// A wrapper for a WASI `stream<u8>` that provides an implementation of
/// `AsyncRead`.
#[derive(Debug)]
...
/// A wrapper for WASI's `output-stream` resource that provides implementations
/// of `AsyncWrite` and `AsyncPollable`.
#[derive(Debug)]
